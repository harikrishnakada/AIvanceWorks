...
This constitution governs every `/services/*` and `/solutions/*` page on the ${SITE_CONFIG.name} website. It ensures every page is produced through a deliberate, repeatable workflow and the component/pattern library stays coherent as it scales.
...
**Who it's for.** Buyers hiring ${SITE_CONFIG.name} to build something. Examples: MVP Development, Full-Stack Development, Cloud Migration, Data Engineering.
...
These rules exist because ${SITE_CONFIG.name} sells trust. A single fabricated stat, discovered by a savvy prospect or a regulator, destroys years of relationship-building. These are hard rules: violations block merge.
...
2. Run it through at least these three challenges:
   ...
   - **Greenfield integrity test** (while ${SITE_CONFIG.name} has no real clients): Can this claim be backed without fabricated outcomes? If not, what's the capability-framed version?
...
- **v2.14 — New single-use semantic-layer comparison signature for Data Analytics + a small reusable schema addition (`benefitsHeading`), Archetype B (Technical service).** Driven by `/services/data-analytics`, the intelligence-layer sibling of `/services/data-engineering`. **The boundary is the whole point:** Data Engineering ends at the Gold layer (curated, trusted data); Data Analytics starts there and builds the *intelligence* layer — BI/dashboards, the semantic/metrics layer, self-service, and predictive insight — never pipelines, warehouse provisioning, medallion architecture, dbt transformation models, governance infra, or streaming (those stay owned by Data Engineering per the page brief's explicit "what to leave out"). The DE→DA handoff is surfaced in the hero subhead, the signature (built on the Gold layer), engagement tier 1 (a data-readiness assessment), a dedicated FAQ, and the Data Engineering `relatedPages` card. **New single-use signature `SemanticLayerModel`** (comparison pattern §8.3 #4 fused with an architectural layer diagram #2): on the *same* Gold layer, WITHOUT a governed semantic layer four BI surfaces (Power BI/Looker/Tableau/finance sheet) re-derive a metric and show four different numbers; WITH analytics engineering, one governed definition feeds every surface identically. A metric selector (MRR / Active User / Churn) swaps the definition and the numbers on both sides. Its argument — *define each metric once and five dashboards stop showing five different revenue numbers* — is a divergence-vs-consistency contradiction a `FeatureGrid` (a flat list of peers) structurally cannot express, and it is the page's strongest differentiator (most analytics firms skip this layer). Distinct from `DataPipelineBlueprint` (data-engineering's 4-stage Ingest→Store→Transform→Serve pipeline): that is a linear pipeline architecture; this is a two-world before/after on one foundation. New hero SVG illustration `DataAnalyticsHeroIllustration` (a decision-intelligence dashboard — KPI, bar chart, trend line — rising out of a governed Semantic-layer bar on top of a Gold-layer bar; deliberately the inverse emphasis of the DE hero's left-to-right plumbing flow). **Archetype call:** B (Technical service) — the buyer wants the intelligence layer *built* (running dashboards + semantic layer), not a plan (so not A). **Composition (10 sections, two darks = hero + signature, non-adjacent, CTA accent, §4 rhythm intact):** hero → metricsStrip → featureGrid → signature → benefitsGrid → processTimeline → engagementModels → relatedPages → faq → ctaBlock. **Deviations from Archetype B default (small, §12.4, recorded here + in the data-file header):** (a) `TechStackBlock` **dropped** — the analytics buyer skews more business (CFO, CEO, Head of Analytics) than the pure-technical data-engineering buyer, so per §9.5 a developer-lens technology chip grid is lower-value; tool names (Power BI, Looker, Tableau, Metabase, dbt metrics layer, LookML, Segment/Amplitude/Mixpanel) are woven into feature prose as "built with" instead (same call as `enterprise-software-development`). (b) `BenefitsGrid` **kept** (data-engineering dropped it) to carry the "When Dashboards Aren't Enough" AI differentiator layer — predictive analytics, anomaly detection, natural-language querying, and customer/product ML models — which is distinct from data-engineering's pipeline-intelligence AI angle. (c) `ProcessTimeline` **kept** so the risk-averse "burned by a BI rollout" buyer sees safe sequencing, with the readiness-assessment-first guardrail as step 1. **Small reusable schema addition (backward-compatible):** `BasePageData` gains optional `benefitsHeading?: { eyebrow?; title?; subtitle? }`, and BOTH `ServiceDetailTemplate` and `SolutionDetailTemplate` now pass it through to the shared `BenefitsGrid` (previously the templates rendered `BenefitsGrid` with no header, so a benefits section always appeared unheaded). All existing pages omit the field → `undefined` → identical to prior behavior; only this page uses it so far. **Content integrity (§9):** every metric capability-framed greenfield-honest (Governed · Self-Service · Decision-First · Foundation-Checked), no fabricated stats; all vendor/tool names framed as "we build with"; engagement prices/durations from the brief flagged in `_unverified` for human sign-off (§9.7); the signature's metric values are flagged as ILLUSTRATIVE of divergence-vs-consistency, not client data (precedent v2.10). **Nav:** added to `NAVIGATION.servicesMenu[2]` (Infrastructure Management, directly below Data Engineering — user-directed, because the DE→DA handoff makes nav adjacency the strongest cross-sell) and to the services-page `SERVICE_DESCRIPTIONS` + `iconMap` (`BarChart3`). **Backlink (§10):** Data Engineering swapped its Generative-AI `relatedPages` card for a journey-aware Data Analytics card ("foundation built → the next step is insight"); Data Analytics links Data Engineering (upstream "start there"), ML Development, and Generative AI. No new archetype, no rhythm-rule change; `benefitsHeading` is a minor optional schema field, not a new section.
- **v2.13 — New reusable shared section `IndustryDirectory` + new `industryDirectory` SectionKey, for the flagship "how we build" service page (Archetype B, medium deviation §12.4).** Driven by `/services/enterprise-software-development` — the flagship Enterprise Software Development page that sits ABOVE the industry verticals (services answer "how we build"; `/industry/*` answers "who we build for"). This is the first SERVICE page that must route down into the industry vertical pages, and the existing `RelatedPages` component could not serve it: `RelatedPageItem.pageType` is `'service' | 'solution'` (an industry is neither, so it gets no correct badge), and `relatedPages` is a 2–3 link buyer-journey block, not a 6-vertical directory. Rather than overload `RelatedPages`, a dedicated section was added: new `SectionKey` member `'industryDirectory'`; new types `IndustryDirectoryLink` (`name` / `connector` / `href` / `icon`) and `IndustryDirectoryData` (`eyebrow` / `title` / `highlightText` / `subtitle` / `items`) in `src/types/pages.ts`; new optional field `industryDirectory?: IndustryDirectoryData` on `ServicePageData`; new shared section `src/components/shared/sections/IndustryDirectory.tsx` (light tone, editorial full-width linked rows — mirrors the industry-hub `IndustryServices` component in reverse, i.e. service→industry rather than industry→service). Both templates were updated for the exhaustive `Record<SectionKey, …>`: `ServiceDetailTemplate` renders it from `data.industryDirectory`; `SolutionDetailTemplate` maps it to `() => null` (service-only). Reusable by any future flagship/service page that should route into the verticals, which is why it lives in `shared/sections/`, not `signature/`. **Archetype call:** B (Technical service) — flagship "how we build," buyer mindset "prove you can build it," deliverable is running systems (not a plan, so not A). Cherry-pick from A: a `personaComparison` "Who We Serve" (six C-suite archetypes self-identify), justified because a flagship umbrella page fans out to many personas. `TechStackBlock` and `ProcessTimeline` dropped per Archetype B (the signature carries the "how"; platform names are woven into feature prose as "we build with", greenfield §9.5). **New single-use signature `EnterpriseDeliveryModel`** (process/flow + convergence, §8.3 #3): three delivery disciplines — Spec-First Engineering → Product Engineering (not body-shopping) → AI-Augmented Delivery — flow along a spine and converge into one owned outcome node ("software you can own"). Its argument — *you get the software you needed, not just what you asked for* — is a feed-into-one-outcome relationship a `FeatureGrid` (a flat list of peers) structurally cannot express, and directly answers the buyer's #1 scar (body shops that shipped the ambiguous brief). New hero SVG illustration `EnterpriseSoftwareHeroIllustration` (a bespoke platform assembled from interconnected modules around a central core — the opposite of a rigid off-the-shelf box). Composition (10 sections, two darks = hero + signature, CTA accent, §4 rhythm intact): hero → metricsStrip → featureGrid → signature → personaComparison → benefitsGrid (the "when your enterprise needs intelligence" AI differentiator) → industryDirectory → relatedPages → faq → ctaBlock. **Verticals linked:** all six live `/industry/*` pages (real-estate, retail, manufacturing-supply-chain, logistics, travel-hospitality, food-beverage). `banking` and `healthcare` industry data files exist but are commented out of `INDUSTRY_PAGE_MODULES` (routes do not resolve) so they are deliberately NOT linked. **Content integrity (§9):** every metric capability-framed (Built to Fit · You Own It · Spec-First · AI-Augmented), no fabricated stats/prices/durations; all vendor/platform names (Snowflake/BigQuery/Redshift, AWS/Azure/GCP, Docker/Kubernetes, ERP/CRM/EDI) framed as "we build with"; ownership/IP explicitly the client's; one build-vs-buy FAQ gives the honest "sometimes buy off-the-shelf" answer. Two journey-aware backlinks added (Custom Software Development and Enterprise AI Development each swapped their weakest-fit cross-vertical solution link for Enterprise Software Development, framed as the "zoom out to the flagship" journey); `application-modernization` left as-is (already carries Digital Transformation as its zoom-out link — asymmetric link accepted per §10). Nav: added to `NAVIGATION.servicesMenu[1]` (Software Engineering) and `SERVICE_DESCRIPTIONS`; `Building2` added to the services-page `iconMap`. No new archetype, no rhythm-rule change; `industryDirectory` will be formalized into §5.2/§7 if a second flagship page reuses it.
- **v2.12 — New page TYPE with its OWN bespoke section library: the Industry hub (`/industry/[slug]`), first instance `/industry/healthcare`.** First deliberate extension beyond `/services/*` and `/solutions/*`. An **industry page** is a deep, standalone vertical landing surface that frames an industry's problems, catalogs the AI & technology capabilities, makes compliance prominent, speaks to sub-verticals, lists the systems/standards we work with, and routes into the **relevant service pages** (solution pages are inactive / not surfaced anywhere on the site, so the directory points to live `/services/*` pages — solutions are used as content reference only). **Key design decision (requester-directed): an industry page does NOT reuse the service/solution section library.** It has its own layout language so it visibly stands apart from a solution page, while staying strictly on the **shared theme tokens** (single brand palette, the four tones §3/§4) so it still reads as the same brand. The solution pages are used as *content* reference only, never as component or layout references. The §4 tone-rhythm rules still hold (image hero `dark`; one second `dark` = the capability bento; no adjacent darks; FAQ penultimate; CTA `accent`). **No signature section** (confirmed with requester) — the dark bento carries the visual weight instead. **New standalone data type `IndustryPageData`** (`src/types/pages.ts`) — does NOT extend `BasePageData`; it has its own field set and its own `IndustrySectionKey` union (`hero | pressures | capabilities | compliance | segments | techStandards | services | faq | cta`), so the industry surface is fully decoupled from `SectionKey` and the service/solution templates (no shared `SectionKey` member, no stubs added to those templates). **New industry-only component library** `src/components/industry/` (9 bespoke components, none reused from `shared/sections/`): `IndustryHero` (editorial full-bleed headline + bottom-docked standards ribbon, not inline metric figures), `IndustryPressures` (sticky thesis + numbered divided ledger, not a card grid), `IndustryCapabilities` (dark **asymmetric bento** — two wide feature panels + three balanced panels, deliberately not a uniform card grid), `IndustryCompliance` (bold thesis band + framework rail + safeguard **definition list** of term/detail rows, not cards), `IndustrySegments` (sub-verticals as a divided ledger, not persona cards), `IndustryTechStandards` (two horizontal label→content rails, not the symmetric `TechStackBlock`), `IndustryServices` (cross-links to relevant **service** pages as an editorial **linked directory** of full-width rows, not `RelatedPages` cards), `IndustryFaq` (sticky-header split + plus-to-x divided `<details>` rows, not the bordered-card `FAQ`), `IndustryCta` (full-bleed `accent` band with inverted white button, not the dark-card-on-light `CTABlock`). All use only token-backed classes (§3) and the `Section`/`Container` primitives for tone + width. **New template `IndustryDetailTemplate`** dispatches `IndustrySectionKey` → the industry components. **New content abstraction** `getIndustryPageData` / `getAllIndustryPageSlugs` + `INDUSTRY_PAGE_MODULES` in `src/lib/content.ts`. **New routes** `app/industry/[slug]/page.tsx` (metadata + WebPage/Service/BreadcrumbList/FAQ JSON-LD + `generateStaticParams`) and `app/industry/page.tsx` (industries index). **Nav:** flat `Industries → /industry` link converted to a mega-menu dropdown (`NAVIGATION.industriesMenu`) wired into `Header.tsx` (desktop + tablet) and `MobileMenu.tsx` (accordion); `'Industries'` added to the main-nav exclusion filters. **Content integrity (§9):** healthcare page is capability-framed ONLY — no client names, case studies, testimonials, or measured-outcome metrics; HITRUST/FedRAMP/SOC 2 framed as alignment/architecture targets, flagged in `_unverified`. **Anti-pattern note:** the avoid-card-grid / no-eyebrow-on-every-section discipline is what differentiates this surface — a single hero kicker is the only eyebrow; every other section leads with a plain heading. The "Industry hub" archetype + its bespoke library will be formalized into §5/§6 when a second industry (Insurance / Life Sciences / Manufacturing — all of which already have solution clusters) reuses these components.
