...
export function FAQSection() {
  ...
  return (
    <section className="py-6 sm:py-8 lg:py-12 bg-surface-warm">
      {/* FAQ Schema */}
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-2 sm:space-y-3">
          {faqs.map((faq, index) => (
            <div
              key={index}
              className={`bg-surface-white rounded-2xl border overflow-hidden transition-all duration-300 ${
                openIndex === index
                  ? 'border-accent/30 shadow-glow-faint'
                  : 'border-border hover:border-border-hover'
              }`}
            >
              <button
                onClick={() => toggleFAQ(index)}
                className="w-full flex items-center justify-between p-3.5 sm:p-5 text-left hover:bg-accent/3 transition-colors"
                aria-expanded={openIndex === index}
                aria-controls={`faq-answer-${index}`}
              >
                <span className="font-bold text-text-heading pr-6 text-xs sm:text-sm md:text-base">
                  {faq.question}
                </span>
                <div className={`flex-shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center transition-all duration-300 ${
                  openIndex === index ? 'bg-accent/8 rotate-180' : 'bg-surface-warm'
                }`}>
                  <ChevronDown
              ...
              >
                <div className="px-3.5 sm:px-5 pb-3.5 sm:pb-5 text-text-body leading-relaxed text-xs sm:text-sm md:text-base">
                  {faq.answer}
                </div>
              </div>
            </div>
